...
class RoutingAgent:
    ...
    async def _async_init_components(
        ...
        auth_token = get_identity_token_from_gcloud()
        async with httpx.AsyncClient(timeout=30) as client:
            for address in remote_agent_addresses:
                try:
                    # Generate identity token for each agent address
                    ...
                    card.url = address
                    print(f"🌐 Fetched public card for: {address}")

                    remote_connection = RemoteAgentConnections(
                        agent_card=card, agent_url=address, auth_token=auth_token
                    )
                    self.remote_agent_connections[card.name.lower().replace(" ", "_")] = remote_connection
                    self.cards[card.name] = card
                    print(f"✅ Registered remote agent: {card.name}")
    ...
    async def send_message(
        ...
        """
        agent_name_key = agent_name.lower().replace(" ", "_")
        if agent_name_key not in self.remote_agent_connections:
            raise ValueError(f'{agent_name_key} not found')
        state = tool_context.state
        state['active_agent'] = agent_name
        client = self.remote_agent_connections[agent_name_key]
        ...
        try:
            print("🟡 Sending message to agent with taskId:", task_id)
            ...
            raise  # Reraise to keep traceback
    ...
    weather_info: str,
        ...
        if not weather_info or not airbnb_options:
            return {
        ...
        return {
            ...
            1. **Location and Dates**: {location}, {dates}
